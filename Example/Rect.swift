...
class Rect: CustomStringConvertible {
    ...
    init(points: Point...) {
        if (points.count == 0) {
            ...
            self.x = Interval(lo: points[0].x, hi: points[0].x)
            self.y = Interval(lo: points[0].y, hi: points[0].y)
    ...
    init(center: Point, size: Point) {
        self.x = Interval(lo: center.x - size.x / 2,
                          hi: center.x + size.x / 2)
        self.y = Interval(lo: center.y - size.y / 2,
